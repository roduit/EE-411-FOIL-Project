...
optimizer_creation_functions = {"SGD": make_SGD, "Adam": make_Adam}
    ...
    for noise_ratio in noise_ratio_list:
        ...
        if dataset_name in dataset_classes:
            ...
            train_dataset = DatasetClass(
                ...
                num_samples=constants.NUM_TRAIN_SAMPLES,
            )
            test_dataset = DatasetClass(
                train=False,
                noise_ratio=noise_ratio,
                num_samples=constants.NUM_TEST_SAMPLES,
            )

            train_dataloader = torch.utils.data.DataLoader(
                dataset=train_dataset, batch_size=constants.BATCH_SIZE, num_workers=2
            )
        ...
        for width in width_model_list:
            out.update(
                IPython.display.Pretty(
                    "Training for width " + str(width) + "/" + str(width_model_list[-1])
                )
            )
            # Create model
            if (model, dataset_name) in model_creation_functions:
                ...
                raise ValueError(
                    f"Invalid model-dataset combination: {model}-{dataset_name}"
                )
            # Create optimizer
            ...
            # Evaluate model
            train_loss, acc_train = predict(
                model=cnn, test_dataloader=train_dataloader, device=constants.DEVICE
            )
            test_loss, acc = predict(
                model=cnn, test_dataloader=test_dataloader, device=constants.DEVICE
            )
...
def model_convergence(
    ...
    if dataset_name in dataset_classes:
        DatasetClass = dataset_classes[dataset_name]
        train_dataset = DatasetClass(
            train=True, noise_ratio=noise_ratio, num_samples=constants.NUM_TRAIN_SAMPLES
        )
        test_dataset = DatasetClass(
            train=False, noise_ratio=noise_ratio, num_samples=constants.NUM_TEST_SAMPLES
        )
